// Particle Sphere — Originkit (Framer export), adapted for this project:
//   - TypeScript types stripped (plain JS)
//   - JSX converted to React.createElement (no bundler in this repo)
//   - `import ... from "three"` converted to a cached dynamic import() of the
//     three.js ES module build from a CDN, awaited inside the effect --
//     there's no @react-three/fiber/drei here, this component drives
//     plain three.js directly, same as the sourced component does
//   - Two tuned "potato" modifications applied where marked below; everything
//     else (rotation, drag, cursor repulsion, click/touch scatter, resize)
//     is unmodified from the sourced component.
(() => {
...
function OriginkitParticleSphere(__props) {
  const {
    particlesCount = 10000,
    speed = 20,
    smoothing = 7,
  ...
  const zoomProbeRef = React.useRef(null);
  const sceneRef = React.useRef(null);
  const cameraRef = React.useRef(null);
  const rendererRef = React.useRef(null);
  const particlesRef = React.useRef(null);
  const particlesGroupRef = React.useRef(null);
  const animationFrameRef = React.useRef(null);
  const animateFnRef = React.useRef(null);
  const startAnimationRef = React.useRef(null);
  const lastResizeRef = React.useRef({ ts: 0, zoom: 0, w: 0, h: 0, aspect: 0 });
  const mouseRef = React.useRef(null);
  const baseParticlePositionsRef = React.useRef([]);
  const particleDisplacementsRef = React.useRef([]);
  const particleScatterVelocitiesRef = React.useRef([]);

  // Check canvas mode ONCE at component mount and cache it
  const isCanvasRef = React.useRef(null);
  ...
  React.useEffect(() => {
    ...
    loadThree().then((THREE) => {
      ...
      for (let i = 0; i < particlesCount; i++) {
        // --- POTATO SHAPE ---
        // Tuned values: lumpiness 0.16, elongation 1.18
        const y = 1 - (i / (particlesCount - 1)) * 2;
        const radius = Math.sqrt(1 - y * y);
        const theta = goldenAngle * i;
        const ux = Math.cos(theta) * radius;
        const uy = y;
        const uz = Math.sin(theta) * radius;
        const lump =
          1.0 +
          0.16 * Math.sin(ux * 2.3 + uy * 1.7) +
          0.112 * Math.sin(uy * 3.1 + uz * 2.2) +
          0.08 * Math.sin(uz * 2.8 + ux * 3.4);
        const posX = ux * sphereRadius * lump * 0.78;
        const posY = uy * sphereRadius * lump * 1.18;
        const posZ = uz * sphereRadius * lump * 0.72;
      ...
      if (particleShape === "sphere") {
        ...
        // --- POTATO EYES (SPOTS) ---
        // Tuned values: spotCount 30, spotRadius 0.28
        const spotCount = 30;
        const spots = [];
        for (let s = 0; s < spotCount; s++) {
          ...
          spots.push(new Vector3(Math.cos(st) * sr, sy, Math.sin(st) * sr));
        }
        const spotColor = new Color("#A87A52"); // potato-brown, project palette
        const spotRadius = 0.28;
        const instanceColors = new Float32Array(particlesCount * 3);
        for (let i = 0; i < particlesCount; i++) {
          const idx = i * 3;
          const p = baseParticlePositionsRef.current[i].clone().normalize();
          let inSpot = false;
          for (let s = 0; s < spots.length; s++) {
            if (p.distanceTo(spots[s]) < spotRadius) {
              inSpot = true;
              break;
            }
          }
          const c = inSpot ? spotColor : baseColorObj;
          instanceColors[idx] = c.r;
        ...
        // Sourced as `false`, which silently disabled all per-instance
        // coloring (base sphereColor and the potato-eye spots alike never
        // rendered). Flipped to `true` per Sarah's call so instanceColor
        // actually reaches the material.
          ...
          if (particlesRef.current) {
            ...
            if (particlesRef.current.material) {
              if (Array.isArray(particlesRef.current.material)) {
                particlesRef.current.material.forEach((mat) =>
                  mat.dispose()
                );
              } else {
...
OriginkitParticleSphere.displayName = "Particle Sphere";

window.OriginkitParticleSphere = OriginkitParticleSphere;
